
import MaskImageFlashlight from '@/components/ui/masks/mask-image-flashlight';
import SVGMaskFlashlight from '@/components/ui/masks/svg-mask-flashlight';
import { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Cursor Effects Page',
  description: 'This page contains cool effects for the mouse cursor',
};

const CursorEffectsPage = () => (
  <main className='bg-slate-800 h-screen text-white'>
    <section className='h-full container relative bg-slate-800'>
      <h1 className='text-2xl pt-8'>Cursor Effects</h1>
      <div className='my-4'>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
      </div>
      <div className='my-4'>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde excepturi dicta quaerat eum maiores. Deserunt, ex. Incidunt assumenda dignissimos nostrum provident praesentium iusto, ratione dicta ducimus hic laboriosam, perspiciatis laborum!</p>
      </div>
    </section>
    {/* <MaskImageFlashlight /> */}
    <SVGMaskFlashlight />
  </main>
);

export default CursorEffectsPage;
